	...
	private abstract class DynamicTestHandler implements ITestHandler<UpdatableReachingDefinition> {
		...
		public void patchGraph(int phase) {
			...
			for (SootClass sc : Scene.v().getApplicationClasses()) {
				System.out.println("Marking class for reload: " + sc.getName());
				ac.add(sc);
				allClasses.add(sc);
			}
			if (PATCH_LIBRARIES)
...
//				"-no-bodies-for-excluded",
	...
	private ITestHandler<UpdatableReachingDefinition> ITestHandlerNewVersion() {
		return new DynamicTestHandler("pdfsam-2.2.1.jar", "pdfsam-2.2.2.jar", "pdfsam.jar") {
	...
	/**
	 * Performs a simple analysis and then replaces the PDFsam application with a
	 * newer version
	 */
	@Test
	public void newVersionSH_Rerun() {
		System.out.println("Starting newVersionSH_Rerun...");
		performTestRerun(ITestHandlerNewVersion(), "org.pdfsam.guiclient.GuiClient");
		System.out.println("newVersionSH_Rerun finished.");
	}
	...
	public void newVersionSH_Propagate() {
		System.out.println("Starting newVersionSH_Propagate...");
		performTestDirect(ITestHandlerNewVersion(), "org.pdfsam.guiclient.GuiClient");
		System.out.println("newVersionSH_Propagate finished.");
	}
